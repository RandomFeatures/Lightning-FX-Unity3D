Shader "LightningBoltAnim"
{
	Properties 
	{
		...
		_BoltSharpness("_BoltSharpness", Range(0,10) ) = 6
		_NoiseMovementRate("_NoiseMovementRate", Range(0,2) ) = 0.65
		_NoiseFrequency("_NoiseFrequency", Range(0,2) ) = 1
		_NoiseScale("_NoiseScale", Range(0,3) ) = 1
		_OpacityMult("_OpacityMult", Float) = 1
		_OpacityFalloff("_OpacityFalloff", Float) = 2
		_MovementRateGlobal("_MovementRateGlobal", Float) = 1
		_MovementRateGlobal_Pixel("_MovementRateGlobal_Pixel", Float) = 1
...
#pragma target 3.0
		...
		float _MovementRateGlobal_Pixel;
		float _BoltSharpness;
		float _OpacityMult;
		float _OpacityFalloff;
		float _MovementRateGlobal;
		float _NoiseMovementRate;
		...
		void vert (inout appdata_full v, out Input o) {
			...
			float4 Multiply59=float4( 25) * float4(_MovementRateGlobal);
			...
			float4 Multiply35=float4( 3) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply36=float4( 0.15) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply39=float4( 0.15) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply44=float4( -0.1051) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply47=float4( 0.256) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply50=float4( 0.190731) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply56=float4( 2.705931) * _NoiseMovementRate.xxxx;
			...
			float4 Multiply53=float4( -0.107335) * _NoiseMovementRate.xxxx;
		...
		void surf (Input IN, inout EditorSurfaceOutput o) {
			...
			float4 Multiply13=float4( 25) * float4(_MovementRateGlobal_Pixel);
			...
			float4 Pow1=pow(Multiply9,_BoltSharpness.xxxx);
			...
			float4 Pow0=pow(Abs1,float4(_OpacityFalloff));
			float4 Lerp1=lerp(float4(_OpacityMult),float4( 1),Pow0);
